...
class BaseOptions(object):
    ...
    def parse(self):#加载参数的函数，返回的是参数表self.args
        ...
        with open(os.path.join(self.args.save_path, 'args.yaml')) as f:#打开参数的配置文件，文件路径拼接
            extra_args = yaml.load(f)#提取参数
        self.args = argparse.Namespace(**vars(self.args), **extra_args)#从yaml文件中导入参数
        return self.args
...
def extract_features(loader, model, index_feature=None, return_numpy=True):
    ...
    if type(model) is not tuple:
        ...
        indices_feature = index_feature
    for m in models:
        m.eval()#执行一个字符串表达式，并返回表达式的值,但是这里没有字符串啊
    ...
    for i, data in enumerate(loader):
        ...
        for m, feat_idx in zip(models, indices_feature):
            with torch.no_grad():#强制之后的内容不进行梯度 计算图构建
